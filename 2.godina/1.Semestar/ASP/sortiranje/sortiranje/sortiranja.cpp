...
void Merge(int *array, int *tmp, int lpos, int rpos, int rend) {
	...
	// treba uoèiti da se neæe izvršiti obje while petlje jer je
	// jedno polje sigurno ostalo bez elemenata!
	while (rpos <= rend) {
		tmp[k++] = array[rpos++];
	}
	// i konaèno prekopiraj tih n elemenata natrag u originalno polje
	for (i = 0; i < n; i++, rend--)
		array[rend] = tmp[rend];
}
...
void Podesi(int *array, int n, int i) {
	...
	while (j < n) {
		// ako desno dijete postoji, i ako je veæe od lijevog, tada
		// neka j postane indeks desnog djeteta
		if (j < n - 1 && array[j] < array[j + 1]) j++;
		// ako veæe dijete nije veæe od roditelja, prekid!
		if (t >= array[j]) break;
		// Inaèe treba popraviti strukturu. Zato u roditelja kopiramo veæe dijete,
		// a u veæe dijete bismo trebali roditelja: t, no ako æe i ispod trebati
		// mijenjati, onda smo bezveze kopirali pa to ne radimo veæ pamtimo da je to t.
		// Ujedno proglašavamo veæe dijete roditeljem, i raèunamo indeks njegove djece.
		array[i] = array[j]; i = j; j = 2 * j + 1;
	}
	// sada konaèno stavljamo ovu zapamæenu vrijednost na mjesto bivšeg veæeg djeteta.
	array[i] = t;
}
